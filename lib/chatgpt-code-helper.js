...
export default {
    ...
    async fetch() {
        ...
        if (editor = atom.workspace.getActiveTextEditor()) {
            const userMessage = editor.getSelectedText();
            const bracketMatcherSettings = atom.config.get('bracket-matcher');
            const bracketMatcherEnabledByUser = bracketMatcherSettings.autocompleteBrackets;
            if(bracketMatcherEnabledByUser)
                bracketMatcherSettings.autocompleteBrackets = false;
            if (userMessage) {
                ...
                try {
                                ...
                                completeMessages.split("\n").forEach((messageStr) => {
                                    ...
                                    try {
                                        ...
                                        if (json.choices && json.choices[0].delta && json.choices[0].delta.content) {
                                            ...
                                            editor.insertText(json.choices[0].delta.content);
                                        }
                                    } catch (e) {
                    ...
                    if(bracketMatcherEnabledByUser)
                        bracketMatcherSettings.autocompleteBrackets = true;
                } catch (error) {
                    if(bracketMatcherEnabledByUser)
                        bracketMatcherSettings.autocompleteBrackets = true;
